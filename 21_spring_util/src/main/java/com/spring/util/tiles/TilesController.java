...
public class TilesController {
	...
	public String content1() {
		return "/content1";	 // tiles설정 파일의 <definition name="/content1">로 리턴이 된다.
	}
	@RequestMapping(value = "/content2", method=RequestMethod.GET)
	public ModelAndView content2() {
		ModelAndView mv = new ModelAndView();
		mv.setViewName("/content2");
		
		return mv; // tiles설정 파일의 <definition name="/content2">로 리턴이 된다.
	}
	@RequestMapping(value = "/content3", method=RequestMethod.GET)
	public ModelAndView content3() {
		return new ModelAndView("/content3"); // tiles설정 파일의 <definition name="/content3">로 리턴이 된다.
	}
